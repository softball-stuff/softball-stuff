...
export default function setupRooms(timeslots: number, rooms: Room[]) {
  const timetableTemplate: Timetable = rooms
    .map(r => ({ [r.id]: [] }))
    .reduce((acc, e) => ({ ...acc, ...e }));
  return function setupGuests(guests: Guest[]) {
    return function setupConstraints(
      constraintInitializers: ConstraintInitializer[]
    ) {
      function* generateSchedules(guestSorter?: GuestSorterHeuristic) {
        ...
        const internalGuestMap = internalGuests
          .map(g => ({ [g.id]: g }))
          .reduce((acc, e) => ({ ...acc, ...e }));
        ...
        for (let i = 0; i < timeslots; i++) {
          ...
          currentTimetable =
            userInput || JSON.parse(JSON.stringify(timetableTemplate));
          if (!userInput) {
            const sortedGuests = [...internalGuests].sort(guestSorter);

            for (const guest of sortedGuests) {
              const { satisfied, unsatisfied } = checkConstraints(
                guest,
                constraints
              );
